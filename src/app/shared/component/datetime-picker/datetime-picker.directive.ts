...
export class DateTimePickerDirective implements OnInit, OnChanges {
  ...
  /* input element */
  private datetimePickerEl: HTMLElement;
  /* dropdown element */
  private componentRef: ComponentRef<DateTimePickerComponent>;
  ...
  hideDatetimePicker = (event?): void => {
    if (this.componentRef) {
      ...
      event &&
      event.type === 'click' &&
      event.target !== this.el && !this.elementIn(event.target, this.datetimePickerEl)
      ) {
